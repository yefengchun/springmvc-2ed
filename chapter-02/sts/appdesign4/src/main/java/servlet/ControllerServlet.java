...
@WebServlet(name = "ControllerServlet", urlPatterns = { "/form", "/pdf" })
public class ControllerServlet extends HttpServlet {
	...
	protected void process(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * /app10a/product_input. However, in the case of a default context, the context
		 * name is empty, and uri has this form /resourceName, e.g.: /product_input
		 */
		...
		if ("form".equals(action)) {
			...
			String sessionId = session.getId(); System.out.println("session id="+sessionId);
			PDFAction pdfAction = (PDFAction) dependencyInjector.getObject(PDFAction.class);
			String text = request.getParameter("text"); System.out.println("text=" + text);
